...
const platillos = () => {
  ...
  platillos.forEach(
    (platillo) => (platillosArrego = [...platillosArrego, platillo])
  );

  const ensaladas = platillosArrego.filter(
    (ensalada) => ensalada.getAttribute("data-platillo") === "ensalada"
  );

  const pastas = platillosArrego.filter(
    (pasta) => pasta.getAttribute("data-platillo") === "pasta"
  );

  const pizzas = platillosArrego.filter(
    (pizza) => pizza.getAttribute("data-platillo") === "pizza"
  );

  const postres = platillosArrego.filter(
    (postre) => postre.getAttribute("data-platillo") === "postre"
  );

  mostrarPlatillos(ensaladas, pastas, pizzas, postres, platillosArrego);
};
